...
class Indexer(object):
    ...
    def get_current_alias(self):
        aliasses = list(es.indices.get_alias(name=self.name, ignore_unavailable=True).keys())
        if aliasses:
            return aliasses[0]
    ...
    def finish(self):
        ...
        if es.indices.exists_alias(name=self.name):
            # Get alisases
            aliased_indices = es.indices.get_alias(name=self.name, ignore_unavailable=True).keys()
